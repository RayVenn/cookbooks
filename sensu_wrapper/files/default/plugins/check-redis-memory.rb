...
class RedisChecks < Sensu::Plugin::Check::CLI
  ...
  def run
    ...
    warn_memory = ((%x(free).split(" ")[7]).to_i)*(config[:warn_mem].to_f)
    crit_memory = ((%x(free).split(" ")[7]).to_i)*(config[:crit_mem].to_f)
    if used_memory >= crit_memory
     ...
     ok "Redis memory usage is below defined limits - used #{used_memory} KB"
    end
  rescue
