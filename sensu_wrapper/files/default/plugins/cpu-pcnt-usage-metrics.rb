...
class CpuGraphite < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
    ...
    user_val = sprintf('%.02f', (cpu_sample_diff["user"] / cpu_total_diff.to_f) * 100)
    system_val = sprintf('%.02f', (cpu_sample_diff["system"] / cpu_total_diff.to_f) * 100)
    idle_val = sprintf('%.02f', (cpu_sample_diff["idle"] / cpu_total_diff.to_f) * 100)

    output "#{config[:scheme]}.user", user_val
    output "#{config[:scheme]}.system", system_val
    output "#{config[:scheme]}.idle", idle_val
